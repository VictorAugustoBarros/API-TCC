...
class ArangoDB:
    default_arango_collections = {"Objetivos": {"edge": False}}

    def __init__(self, collection: str, edge: bool = False):
        self.collection = collection
        conn = Connection(
            arangoURL=f"{arango_host}:{arango_port}",
        ...
        for (
            collection_name,
            collection_values,
        ) in self.default_arango_collections.items():
            self.create_collection(
                collection_name=collection_name, edge=collection_values.get("edge")
            )
