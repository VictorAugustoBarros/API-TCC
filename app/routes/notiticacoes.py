...
@routes_notificacoes.get("/api/notificacao")
@token_validation
async def get_solicitacao_amizade(request: Request):
    try:
...
@routes_notificacoes.get("/api/notificacao/friend_request")
@token_validation
async def find_friend_request(request: Request):
    try:
        user_data = request.state.token
...
@token_validation
async def delete_notificacao(request: Request):
    try:
...
@routes_notificacoes.delete("/api/notificacao")
@token_validation
async def delete_notificacoes(request: Request):
    try:
        user_data = request.state.token

        notificacoes_model = NotificacoesModel()
        notificacoes_model.delete_all_notificacao(user_key=user_data.get("key"))
...
@routes_notificacoes.post("/api/notificacao")
@token_validation
async def get_user_card(request: Request):
    try:
        user_data = request.state.token
