...
class UserCriteriosModel(ArangoDB):
    def __init__(self):
        super().__init__(collection="UserCriterios")

    def insert_criterio(self, user_criterio: UserCriterio):
        self.insert(**user_criterio.__dict__)

    def find_criterio_by_user(self, user_id: str):
        criterios = self.find(data={"user_id": user_id})
        if criterios:
            return criterios[0]
        return criterios
